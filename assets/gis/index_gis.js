...
// MAIN
    ...
    // FOR MARKERS
        ...
        showPOI(main,markMain,mainLayer);
        showPOI(makan,markMakan,mkanLayer);
        showPOI(wa,markWisata,wisaLayer);
        showPOI(hotel,markHotel,hotlLayer);
        showPOI(sejarah,markHerit,sejrLayer);
        showPOI(oleh,markOleh,olehLayer);
        ...
        for(var i=0; i<pusat.length; i++) {
            marker_pk = new L.Marker.Text(new L.LatLng(pusat[i][1],pusat[i][2]), pusat[i][0], {icon: ico_pk})
                .addTo(map);
        }